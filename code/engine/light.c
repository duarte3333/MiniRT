...
static int	shadow_light(t_raytracer *rt)
{
	t_object	*obj;
	t_raytracer	new_rt;
...
static void	specular_light(t_object *tmp, t_raytracer *rt)
{
	if (rt->rl.s)
	{
...
static void	diffuse_light(t_object *tmp, t_raytracer *rt)
{
...
int	point_and_directional_light(t_raytracer *rt, t_object **tmp)
{
	...
	diffuse_light((*tmp), rt);
	specular_light((*tmp), rt);
	return (0);
}
