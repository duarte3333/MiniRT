...
/*   Updated: 2023/10/12 21:31:10 by duarte33         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	create_chunks(int i)
{
	...
	if ((i == vars()->n_threads - 1) && rest != 0)
	{
		...
		vars()->threads[i].x_f = vars()->threads[i].delta + \
			vars()->threads[i].x_i;
		(vars())->threads[i].color = ft_calloc(sizeof(int), (int)(HEIGHT) \
			*vars()->threads[i].delta);
	}
	else
	{
...
int	ft_init_threads(void)
{
	...
	while (++i < vars()->n_threads)
	{
		...
		if (pthread_create(&vars()->threads[i].thread, \
			NULL, &routine, (void *)(&threads[i])) != 0)
		{
