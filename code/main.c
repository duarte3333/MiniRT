...
/*   Updated: 2023/10/12 20:25:35 by duarte33         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	create_scene(char *arg)
{
	...
	return (create_scene_helper(&head, &end_scene, fd));
}
...
int	main(int ac, char **av)
{
	...
	if (ac >= 2)
	{
		...
		if (ft_init_threads() == -1)
			return (-1);
		mlx_loop_hook(vars()->mlx, (void *)paint, NULL);
		mlx_loop(vars()->mlx);
...
//Ver bases do cone e cilindro DONE
